...
voxelSize = 0.05; % voxel size for approx. QC. can be [] for the non-approx algorithm
stochasticSz = 0.05; % gradient will be estimated based on this relative part of the data points, chosen at random each step for each replica. can be [] for the non-stochastic algorithm. Otherwise should be in range (0,1]
howOftenToTestIfDone = 50; % how often to perform a check of whether stop condition of replica is fullfilled? This could be 1, unless using stochasticSz and then this should be higher
