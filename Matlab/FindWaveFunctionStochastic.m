function [Psi,dPsi] = FindWaveFunctionStochastic(data,sigma,x,sz)
% finds the estimated wave function\Parzen function and its gradient, using a sample of data points.
...
% sz - a number between 0 and 1. The wave function and gradient will be calculated based on a random sample of size 'sz*size(data,1)' of data points
...
% dV - the gradient of the wave function at the points x. It has the same size as x.
	...
	for ii=1:size(x,1)
		...
		dPsi(ii,:) = -1*sum(difference.*repmat(gaussian,1,size(currentData,2))*2*sigma^2);
	end	
end