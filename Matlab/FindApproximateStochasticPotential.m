function [V,dV] = FindApproximateStochasticPotential(data,coeff,sigma,x,sz)
% finds the estimation to the approximate potenatial and its gradient for quantum clustering
...
% dV - the gradient of the potential at the points x. It has the same size as x. 
	...
	for ii=1:size(x,1)
		...
		dV(ii,:) = (1/parzen)*sum(difference.*repmat(coeff.*gaussian,1,size(currentData,2)).*(2*sigma^2*V(ii)-repmat(squaredDifference,1,size(currentData,2))));
	end	
