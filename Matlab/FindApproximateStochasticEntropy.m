function [S,dS] = FindApproximateStochasticEntropy(data,coeff,sigma,x,sz)
% finds the estimated approximate entropy and its gradient for quantum clustering
...
% dS - the gradient of the entropy at the points x. It has the same size as x. 
	...
	for ii=1:size(x,1)
		...
		dS(ii,:) = (1/parzen)*sum(difference.*repmat(coeff.*gaussian,1,size(currentData,2)).*(2*sigma^2*V-repmat(squaredDifference,1,size(currentData,2))));
	end	
